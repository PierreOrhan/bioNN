...
class pythonSolver():
    def __init__(self,masks,k1,k1n,k2,k3,k3n,k4,k5,k5n,k6,kdI,kdT,TA,TI,E0):
        ...
        self.kdT = [np.zeros(m.shape)+kdI for m in masks]
        self.kdI = [np.zeros(m.shape)+kdT for m in masks]

        self.TA0 = [np.where(m>0,TA,0) for m in masks]
        self.TI0 = [np.where(m<0,TI,0) for m in masks]
